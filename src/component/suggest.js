...
export default class Suggest {
  ...
  search(word) {
    let { items, itemsAll } = this;
    if (itemsAll.length > 1000 && word) {
      items = itemsAll;
    }
    if (!/^\s*$/.test(word)) {
      items = items.filter(it => (it.title || it).includes(word.toUpperCase()));
    }
    items = items.map((it) => {
      let { title } = it;
      if (title) {
    ...
    this.filterItems = items.slice(0, 1000);
    const { el } = this;
    if (items.length <= 0) {
      el.html('').show();
      return;
